...
def gpu_check() -> torch.device:
    ...
    if not cuda_available:
        print("  CUDA is NOT available. Falling back to CPU for UNet model training.")
        print("=" * 65)
...
def _train_epoch_local(
    ...
    use_amp_cuda = cfg.use_amp and device.type == "cuda"
...
def local_train(cfg: LocalTrainConfig) -> list[dict[str, float | int]]:
    ...
    for epoch in range(start_epoch, cfg.num_epochs + 1):
        ...
        # ── Print progress ──
        vram_mb = (torch.cuda.memory_allocated(device) / (1024 ** 2)) if device.type == "cuda" else 0.0
        print(
