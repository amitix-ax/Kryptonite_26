...
def astar(ice_grid: np.ndarray, start: tuple[int, int], goal: tuple[int, int], safety_weight: float):
    ...
    while open_set:
        ...
        for dr, dc, mv in _DIRS:
            ...
            # Ice resistance & kinematic effective speed
            v_eff = max(0.5, 14.0 - (12.0 * (sic ** 1.5)) + wind_boost + ocean_assist + drift_boost)
            
            # Non-linear hazard cost penalty for ice concentration > 5%
            ice_penalty = 1.0 + 35.0 * (max(0.0, sic - 0.05) ** 2.2) * (safety_weight + 0.5)
            
            # Physical distance in NM (~7NM per cell at this resolution)
            dist_nm = mv * 7.0 
            time_cost = (dist_nm / v_eff) * ice_penalty
...
def compute_route(
    ...
    if slat > LAT_MAX:
        entry_lat = LAT_MAX
        frac = (LAT_MAX - slat) / (elat - slat) if (elat - slat) != 0 else 0.5
        entry_lon = slon + frac * (elon - slon)
    ...
    if len(unique_rc) > 5:
        ...
        c = rc_arr[:, 1]
        try:
            k_val = min(3, len(unique_rc) - 1)
            tck, u = si.splprep([r, c], s=0.5, k=k_val)
            # Generate highly detailed micro-grid points
            u_new = np.linspace(0, 1.0, max(200, len(unique_rc) * 2))
